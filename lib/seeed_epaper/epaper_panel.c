...
esp_err_t seeed_epaper_panel_wait_ready(seeed_epaper_panel_t *panel)
{
    ...
    const uint64_t start_ms = epaper_log_ms();
    vTaskDelay(pdMS_TO_TICKS(50)); // Wait for SSD1677 to assert BUSY

    while (gpio_get_level(panel->config.pin_busy) == (int)panel->config.busy_level) {
        vTaskDelay(pdMS_TO_TICKS(20));
        if (epaper_log_ms() - start_ms > panel->config.busy_timeout_ms) {
            ESP_LOGE(TAG, "busy wait timeout after %llums", (unsigned long long)(epaper_log_ms() - start_ms));
            return ESP_ERR_TIMEOUT;
        }
    }
    ESP_LOGI(TAG, "busy wait completed in %llums", (unsigned long long)(epaper_log_ms() - start_ms));
    return ESP_OK;
}
